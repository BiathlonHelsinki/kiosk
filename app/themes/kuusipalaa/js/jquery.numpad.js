			...
			nmpd.close = function(target){
				...
				if (target){
					...
					if (target.prop("tagName") == 'INPUT'){
						...
						// $('#enter_pin_hidden').val(nmpd.getValue().toString().replace('.', options.decimalSeparator))
					} else {
						target.html(nmpd.getValue().toString().replace('.', options.decimalSeparator));
					}
				}	
				$('input#enter_pin').val('Change PIN')
				...
				$('#lower').html('Place an ID card against the reader and click the link button above to connect the card to this account.')
